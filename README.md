...
<p align="center">
  <img src="streamlit-app.png" alt="Fraud detection Streamlit app" width="850">
</p>

<p align="center">
  <a href="https://pbl-datascience-baf2022-8lmr9vkarc7mfckfaea8hy.streamlit.app/"><strong>Live Demo →</strong></a> · <a href="BAF-PBL.ipynb"><strong>Full EDA/Modeling Notebook →</strong></a>
</p>
...
### Why this dataset
BAF was built specifically to fill a gap the fraud/fairness-ML research community had flagged: most public fraud datasets (e.g. the well-known ULB "Credit Card Fraud Detection" set on Kaggle) are small, fully anonymized (PCA-transformed features), and have no fairness-relevant fields. They're useful for basic imbalanced-classification practice, but not for auditing bias. BAF's NeurIPS 2022 paper ("Turning the Tables: Biased, Imbalanced, Dynamic Tabular Datasets for ML Evaluation," Jesus et al., Feedzai Research) was published specifically to give the field a **large-scale, temporally-realistic, fairness-labeled** benchmark. It's cited in follow-up fairness research (e.g. Fairness-Aware Data Valuation, 2023) and recommended in ML community discussions as one of the few datasets suited to account-opening fraud and fairness testing specifically, which is why it was chosen here over the more common but fairness-blind alternatives.
...
- Linear correlations between individual features and fraud are all weak in isolation (Pearson r: credit_risk_score 0.071, customer_age 0.063, income 0.045, velocity_6h −0.017), confirming that fraud in this dataset is driven by **feature interactions**, not any single strong linear predictor. This is exactly why a tree-based ensemble (HistGradientBoosting) outperforms simpler linear models.
...
## Deployment
6 trained artifacts were exported for production use: the HistGradientBoosting model, the fitted `StandardScaler`, the chosen decision threshold, the final feature list, the scaled-column list, and a median-value template for handling missing input fields. These were then wired into the Streamlit app shown above.

## Recommendation
Deploy with active fairness monitoring, not just accuracy monitoring. The 8.7x FPR gap needs mitigation before production use. Because fraud patterns drift over time, the model should be retrained and monitored on a rolling basis rather than treated as a one-time deployment.
...
**[Try it live →](https://pbl-datascience-baf2022-8lmr9vkarc7mfckfaea8hy.streamlit.app/)**
